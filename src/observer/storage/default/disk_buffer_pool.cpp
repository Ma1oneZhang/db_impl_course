...
Frame *BPManager::alloc(int file_desc, PageNum page_num) {
   ...
   */
	if (lrucache.size() == size) {
		BufferTag VictimEd = {};
		auto res = lrucache.getVictim(&VictimEd, not_pinned, (void *) this);
		if (res != RC::SUCCESS) {
			return nullptr;
		}
		auto idx = 0;
		// 一定存在
		lrucache.get(VictimEd, &idx);
		disk_buffer_pool->flush_block(&frame[idx]);
		BPPageHandle handle = {
				true,
				&frame[idx]};
		disk_buffer_pool->get_this_page(file_desc, page_num, &handle);
		lrucache.victim(VictimEd, {file_desc, page_num});
		return &frame[idx];
	} else {
		int idx = -1;
		...
		disk_buffer_pool->get_this_page(file_desc, page_num, &handle);
		allocated[idx] = true;
		lrucache.put({file_desc, page_num}, idx);
		return &frame[idx];
	}
}
